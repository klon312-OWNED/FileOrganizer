...
class App(Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Файловый менеджер — сортировка по типу и дате")
    ...
    def _build_ui(self) -> None:
        ...
        status = Label(self, textvariable=self.status_var, anchor="w", padx=8,
                       pady=4, relief="sunken")
        status.pack(side="bottom", fill=X)

        # Вкладки: архив и поиск по всему ПК
        nb = ttk.Notebook(self)
        nb.pack(side="top", fill=BOTH, expand=True)
        tab_archive = Frame(nb)
        tab_pc = Frame(nb)
        nb.add(tab_archive, text="  Архив (отсортированное)  ")
        nb.add(tab_pc, text="  Поиск по всему ПК  ")
    ...
    def _build_archive_tab(self, root) -> None:
        ...
        toolbar = Frame(root, padx=8, pady=8)
        toolbar.pack(side="top", fill=X)

        ttk.Button(toolbar, text="Сортировать сейчас", command=self._sort_now).pack(side=LEFT)
        ttk.Button(toolbar, text="Отменить последнюю", command=self._undo_last).pack(side=LEFT, padx=(6, 0))
        ttk.Button(toolbar, text="Обновить индекс", command=self._reindex).pack(side=LEFT, padx=(6, 0))

        self.bg_var = StringVar(value="Фоновый режим: выкл")
        self.bg_btn = ttk.Button(toolbar, textvariable=self.bg_var, command=self._toggle_watcher)
        self.bg_btn.pack(side=LEFT, padx=(6, 0))

        ttk.Button(toolbar, text="Настройки", command=self._open_settings).pack(side=LEFT, padx=(6, 0))

        Label(toolbar, text="Поиск:").pack(side=LEFT, padx=(16, 4))
        ...
        # Основная область: слева фильтры, справа таблица
        body = Frame(root)
        body.pack(side="top", fill=BOTH, expand=True, padx=8, pady=(0, 8))

        sidebar = Frame(body, width=210)
        sidebar.pack(side=LEFT, fill=Y, padx=(0, 8))
        sidebar.pack_propagate(False)

        Label(sidebar, text="Категория", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        ...
        Label(sidebar, text="Год", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        ...
        Label(sidebar, text="Месяц", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        ...
        preview = Frame(body, width=320)
        preview.pack(side=RIGHT, fill=Y, padx=(8, 0))
        preview.pack_propagate(False)
        Label(preview, text="Предпросмотр", font=("Segoe UI", 10, "bold")).pack(anchor="w")
        img_box = Frame(preview, height=300, bg="#2b2b2b", relief="groove", bd=1)
        img_box.pack(fill=X, pady=(4, 8))
        ...
        Label(preview, textvariable=self.preview_info, justify="left", anchor="nw",
              wraplength=300).pack(fill=BOTH, expand=True, anchor="nw")
    ...
    def _after_sort(self, moved: int):
        self._refresh_filters()
        self._reload_table()
        messagebox.showinfo("Готово", f"Отсортировано файлов: {moved}")
    ...
    def _toggle_watcher(self):
        if self.watcher.running:
            self.watcher.stop()
            self.bg_var.set("Фоновый режим: выкл")
        else:
            self.watcher.start()
            self.bg_var.set("Фоновый режим: ВКЛ")

    def _on_background_sorted(self, new_path: str):
        # вызывается из фонового потока — обновляем интерфейс в главном потоке
    ...
    def _open_settings(self):
        ...
        Label(win, text="Где создать папку «Архив» (внутри неё появятся подпапки):",
              font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=12, pady=(12, 2))
        dest_frame = Frame(win)
        ...
        dest_entry = ttk.Entry(dest_frame, textvariable=loc_var)
        ...
        name_frame.pack(fill=X, padx=12, pady=(6, 0))
        Label(name_frame, text="Имя папки-архива:").pack(side=LEFT)
        ttk.Entry(name_frame, textvariable=name_var, width=24).pack(side=LEFT, padx=(6, 0))

        result_var = StringVar()
        result_lbl = Label(win, textvariable=result_var, fg="#1565c0")
        result_lbl.pack(anchor="w", padx=12, pady=(6, 0))
        ...
        sort_folders_var = StringVar(
            value="on" if self.settings.sort_folders else "off"
        )
        chk = ttk.Checkbutton(
            win, text="Сортировать и папки тоже (переносить целиком в «Папки»)",
            variable=sort_folders_var, onvalue="on", offvalue="off",
        )
        chk.pack(anchor="w", padx=12, pady=(8, 0))

        Label(win, text="Отслеживаемые папки (по одной в строке):",
              font=("Segoe UI", 10, "bold")).pack(anchor="w", padx=12, pady=(14, 2))
        folders_text = Text(win, height=8)
        folders_text.pack(fill=BOTH, expand=True, padx=12)
        ...
        def save():
            ...
            self.settings.data.pop("destination", None)  # убираем устаревший ключ
            self.settings.data["watched_folders"] = folders
            self.settings.save()
            restart = self.watcher.running
            if restart:
                self.watcher.stop()
                self.watcher.start()
        ...
        ttk.Button(btns, text="Отмена", command=win.destroy).pack(side=RIGHT, padx=(0, 6))
