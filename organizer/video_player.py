...
_SPEED_OPTIONS = (0.5, 1.0, 1.5, 2.0)
...
def _fourcc_label(code: int) -> str:
    if code == 0:
        return ""
    try:
        raw = int(code)
        chars = "".join(chr((raw >> (8 * i)) & 0xFF) for i in range(4))
        return chars.strip("\x00") or f"0x{raw:08x}"
    except (TypeError, ValueError):
        return ""


def probe_video_failure(path: Path) -> str:
    """Краткая причина, почему встроенный плеер не открыл файл."""
    if not _HAS_CV2:
        return "модуль OpenCV (cv2) недоступен"
    if not _HAS_PIL:
        return "модуль Pillow недоступен"
    p = Path(path)
    if not p.is_file():
    ...
    if not cap.isOpened():
        cap.release()
        return "файл не распознан как видео"
    try:
        fps = float(cap.get(cv2.CAP_PROP_FPS) or 0)
        frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT) or 0)
        fourcc = _fourcc_label(int(cap.get(cv2.CAP_PROP_FOURCC) or 0))
        ok, frame = cap.read()
        if not ok or frame is None:
            if fourcc:
                return f"кодек «{fourcc}» не поддерживается встроенным декодером"
            return "кодек не поддерживается встроенным декодером"
        if fps <= 0 and frames <= 0:
            return "не удалось определить длительность видео"
    except Exception:
        return "ошибка чтения видеопотока"
    finally:
        cap.release()
...
class VideoPlayer(Frame):
    """Виджет воспроизведения видео: play/pause, скорость, перемотка."""
    ...
    def __init__(self, parent, *, max_size: tuple[int, int] = (360, 280)) -> None:
        ...
        speed_box = ttk.Combobox(
            controls, textvariable=self._speed_var, state="readonly", width=4,
            values=[f"{s:g}×" for s in _SPEED_OPTIONS],
        )
        speed_box.pack(side=LEFT, padx=(2, 0))
        speed_box.bind("<<ComboboxSelected>>", self._on_speed_change)

        Label(
            controls, text="🔇", bg=theme.IMAGE_PREVIEW_BG, fg=theme.TEXT_MUTED,
            font=("Segoe UI", 9),
    ...
    def _frame_delay_ms(self) -> int:
        effective_fps = self._fps * self._speed
        return max(1, int(1000 / effective_fps))

    def _schedule_frame(self) -> None:
        if not self._playing or self._cap is None:
            return
        self._after_id = self.after(self._frame_delay_ms(), self._next_frame)
    ...
    def _on_speed_change(self, _event=None) -> None:
        raw = self._speed_var.get().replace("×", "").strip()
        try:
            speed = float(raw)
        except ValueError:
            speed = 1.0
        if speed not in _SPEED_OPTIONS:
            speed = 1.0
        self._speed = speed
